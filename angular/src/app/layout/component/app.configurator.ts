    ...
    updateColors(event: Event, type: 'primary' | 'surface', color: SurfacesType) {
        if (type === 'primary') {
            ...
            if (isPlatformBrowser(this.platformId)) {
                const surface = this.surfaces.find((s) => s.name === color.name);
                if (surface && surface.palette) {
                    Object.keys(surface.palette).forEach((key) => {
                        const paletteKey = key as unknown as keyof typeof surface.palette;
                        document.documentElement.style.setProperty(
                            `--surface-${key}`,
                            surface.palette[paletteKey] || ''
                        );
                    });
                }
            }
        ...
        const currentPreset = presets[this.layoutService.layoutConfig().preset as KeyOfType<typeof presets>];
        const presetOptions = currentPreset?.options as Record<string, any> | undefined;
        
        this.config.theme.update((theme) => ({
            ...theme,
            ...(currentPreset || {}),
            options: {
                ...theme?.options,
